    ...
    */
    ...
    public class Vendible : Relation
    {
        ...
        public new class Kind : Relation.Kind
        {
            ...
            {
                internal static readonly SqlPrepared FindByCombinedIndexAndLevel;
                internal static readonly SqlPrepared FindByParentAndName;
                internal static readonly SqlPrepared FindByLevel;

                internal const string CombinedIndexField = "combinedIndex";
                internal const string LevelField = "level";
                internal const string GroupedInField = "grp";
                internal const string NameField = "name";

                static PreparedStatements()
                {
                    Vendible.Kind vendibleKind = Kind.GetInstance<Vendible.Kind>();
                    VendibleLevel.Kind level = Kind.GetInstance<VendibleLevel.Kind>();
                    string combinedIndexBase = string.Format(
                        "SELECT v FROM {0} v WHERE v.CombinedIndex=VAR(Int32, {1})",
                        vendibleKind.FullInstanceClassName, 
                        CombinedIndexField);
                    string levelPart = string.Format("v.Level=VAR({0}, {1})",
                        level.FullInstanceClassName, LevelField);
                    
                    FindByCombinedIndexAndLevel = Sql.GetPrepared(combinedIndexBase + " AND " + levelPart);
                    FindByParentAndName = Sql.GetPrepared(string.Format(
                        "SELECT v FROM {0} v WHERE v.GroupedIn=VAR({0}, {1}) AND v.Name=VAR(String, {2})",
                        vendibleKind.FullInstanceClassName,
                        GroupedInField,
                        NameField));

                    string byLevel = string.Format(
                        "SELECT v FROM {0} v WHERE v.Level=VAR({1}, {2})",
                        vendibleKind.FullInstanceClassName,
                        level.FullInstanceClassName,
                        LevelField);
                    FindByLevel = Sql.GetPrepared(byLevel);
                }
            }
            ...
            /// <summary>
            /// Creates a new instance without checking the database if one with the given criterias already exists.
            /// USE WITH CAUTION! (Deprecated)
            /// </summary>
            /// <param name="vendibleObject"></param>
            /// <param name="supplier"></param>
            /// <param name="unit"></param>
            /// <returns></returns>
            public Vendible NewInstance(
                Something vendibleObject, Somebody supplier, UnitOfMeasure unit)
            {
                return NewInstance(vendibleObject, supplier, unit, null, null, null);
            }
            ...
            /// <param name="unit"></param>
            /// <param name="id"></param>
            /// <param name="groupedIn"></param>
            /// <param name="level"></param>
            /// <returns></returns>
            public Vendible NewInstance(Something vendibleObject, Somebody supplier, UnitOfMeasure unit,
                string id, Vendible groupedIn, VendibleLevel level)
            {
                Vendible vendible = New<Vendible>();
                vendible.SetVendibleObject(vendibleObject);
                vendible.SetSupplier(supplier);
                vendible._unit = unit;
                if (!string.IsNullOrEmpty(id))
                    vendible.ID = id;
                if (groupedIn != null)
                    vendible.SetGroupedIn(groupedIn);
                if (level != null)
                    vendible._level = level;
                return vendible;
            }
            ...
            /// <returns></returns>
            public Vendible NewInstance(Something vendibleObject, Somebody supplier, UnitOfMeasure unit,
                Vendible groupedIn, VendibleLevel level)
            {
                Vendible vendible = New<Vendible>();
                vendible.SetVendibleObject(vendibleObject);
                vendible.SetSupplier(supplier);
                ...
                if (groupedIn != null)
                {
                    ...
                    if (groupedIn.Level != null)
                    {
                        vendible._level = groupedIn.Level.LowerLevel as VendibleLevel;
                    }
                }
                if (level != null)
                {
            ...
            /// <summary>
            /// Returns all vendibles with a id matching the given one
            /// </summary>
            /// <param name="productID"></param>
            /// <returns></returns>
            public IEnumerable<Vendible> FindByID(string vendibleID)
            {
                SqlEnumerator<Vendible> e = null;

                if (!string.IsNullOrEmpty(vendibleID))
                {
                    string query = string.Format(
                        "SELECT v FROM {0} v "
                            + "WHERE v.ID=variable(String, id)",
                            FullInstanceClassName);

                    e = Sql.GetEnumerator<Vendible>(query);//, vendibleID);
            ...
            /// <summary>
            /// Finds and returns the groups the matches the criterias.
            /// </summary>
            /// <param name="name">The group name</param>
            /// <param name="level">The level</param>
            /// <returns>A vendible group or null.</returns>
            public IEnumerable<Vendible> FindGroups(string name, VendibleLevel level)
            {
                string query = string.Format(
                    "SELECT v FROM {0} v WHERE v.Name=VAR(String, name) AND"
                    + "                        v.Level=VAR({1}, level)",
                    FullInstanceClassName,
                    level.FullClassName);
                SqlEnumerator<Vendible> e = Sql.GetEnumerator<Vendible>(query); //, name, level);
            ...
            /// <summary>
            /// Returns all vendibles with the given level.
            /// </summary>
            /// <param name="level"></param>
            /// <returns></returns>
            public IEnumerable<Vendible> FindByLevel(VendibleLevel level)
            {
                if (level == null)
                {
                    throw new ApplicationException("A level must be supplied to find all vendibles by that level!");
                }
                string query = string.Format(
                    "SELECT v FROM {0} v WHERE v.Level=VAR({1}, level)",
                    FullInstanceClassName,
                    level.FullClassName);
                SqlEnumerator<Vendible> e = Sql.GetEnumerator<Vendible>(query); //, level);
                e.SetVariable("level", level);
            ...
            /// <param name="level"></param>
            /// <returns></returns>
            public IEnumerable<Vendible> FindByVendibleObject(Something vendibleObject)
            {
                if (vendibleObject == null)
                {
                    throw new ApplicationException("A vendibleObject must be supplied to find all vendibles by with that vendibleObject!");
                }
                return vendibleObject.Roles<Vendible>();
            }

            /// <summary>
            /// 
            /// </summary>
            /// <param name="vendibleObject">Must be supplied</param>
            /// <param name="supplier">Must be supplied</param>
            /// <param name="level"></param>
            /// <param name="unit"></param>
            /// <returns></returns>
            public Vendible Find(Something vendibleObject, Somebody supplier, VendibleLevel level)
            {
                IEnumerator<Vendible> vendibleE = null;
                int combinedIndex = 0;

                if (vendibleObject == null || supplier == null)
                {
                    throw new ApplicationException("Both supplier and vendibleobject must be supplied for finding a single vendible!");
                }


                if (level != null)
                {
                    if (CombinedIndexHelper.TryGet(vendibleObject, supplier, true, out combinedIndex))/*Ok*/
                    {
                        vendibleE = FindVendibleByCombinedIndexAndLevel(level, combinedIndex);
                    }
                }
                else
                {
                    vendibleE = FindVendibleByCombinedIndex(vendibleObject, supplier);
                }

                return CombinedIndexHelper.GetFirst<Vendible>(
                    vendibleE,
                    vendibleObject,
                    supplier);
            }
            public IEnumerable<Vendible> Find(Somebody supplier, VendibleLevel level)
            {
                if (supplier == null || level == null)
                {
                    throw new ApplicationException("Both supplier and level must be supplied for finding a single vendible!");
                }
                SqlEnumerator<Vendible> sql = Sql.GetEnumerator<Vendible>(PreparedStatements.FindByLevel);
                sql.SetVariable(PreparedStatements.LevelField, level);
                foreach (Vendible v in sql)
                {
            ...
            private static IEnumerator<Vendible> FindVendibleByCombinedIndex(
                Something vendibleObject, Somebody supplier)
            {
                return vendibleObject.RelationsTo<Vendible>(supplier).GetEnumerator(); 
            }

            private static IEnumerator<Vendible> FindVendibleByCombinedIndexAndLevel(
                VendibleLevel level, int combinedIndex)
            {
                SqlEnumerator<Vendible> vendibleE = Sql.GetEnumerator<Vendible>(PreparedStatements.FindByCombinedIndexAndLevel);
                vendibleE.SetVariable(PreparedStatements.CombinedIndexField, combinedIndex);
                vendibleE.SetVariable(PreparedStatements.LevelField, level);
                return vendibleE;
            }

            public Vendible Find(Vendible parent, string name)
            {
                Vendible vendible;
                String errorDetail = null;
                if (parent == null) errorDetail = "Parent is null. ";
                if (name == null) errorDetail += "Name is null.";
                if (errorDetail != null)
                {
                    throw new ApplicationException(
                        "Both parent and name must be supplied for Find By Parent. When this was " +
                        "called: " + errorDetail);
                }

                using (SqlEnumerator<Vendible> vendibleE = Sql.GetEnumerator<Vendible>(PreparedStatements.FindByParentAndName))
                {
                    vendibleE.SetVariable(PreparedStatements.GroupedInField, parent);
                    vendibleE.SetVariable(PreparedStatements.NameField, name);
                    vendible = vendibleE.FirstOrDefault<Vendible>(); 
                }
            ...
            /// <param name="level"></param>
            /// <param name="unit"></param>
            /// <param name="smallerPart"></param>
            /// <param name="parent"></param>
            /// <returns></returns>
            public Vendible Assure(Something vendibleObject, Somebody supplier, VendibleLevel level, UnitOfMeasure unit, Vendible smallerPart, Vendible parent)
            {
                Vendible vendible = Find(vendibleObject, supplier, level);

                if (vendible == null)
                {
                    vendible = NewInstance(vendibleObject, supplier, unit, parent, level);
            ...
            public Vendible Assure(Vendible parent, string name)
            {
                Vendible vendible = Find(parent, name);

                if (vendible == null)
                {
                    vendible = this.New<Vendible>();
                    vendible.Name = name;
                    vendible.SetGroupedIn(parent);
                    vendible.SetSupplier(parent.Supplier);

                    if (parent.Level != null)
                    {
                        vendible._level = parent.Level.LowerLevel as VendibleLevel;
                    }
                }
                return vendible;
            }
        ...
        public Something.Kind VendibleObjectKind
        {
            get
            {
                return (VendibleObject is Something.Kind) ? VendibleObject as Something.Kind : VendibleObject.InstantiatedFrom;
            }
        }
        ...
        public IEnumerable<Identifier> Identifiers
        {
            get
            {
                return Kind.GetInstance<Identifier.Kind>().GetIdentifiers<Identifier>(this);
            }
        }
        ...
        public Vendible ConvertToOtherSupplier(Somebody supplier)
        {
            ...
            if (CombinedIndexHelper.TryGet(WhatIs, supplier, false, out combinedIndex))/*Ok*/
            {
