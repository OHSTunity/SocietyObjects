...
namespace Concepts.Ring2
{
    ...
    public class Company : Organisation
    {
        ...
        /// 
        /// </summary>
        public new class Kind : Organisation.Kind
        {
            public virtual Company Assure(string orgregno, string fullname, string description)
            {
                Company company = null;
                if (orgregno != null)
                {
                    using (SqlEnumerator sqlEnumerator = Sql.GetEnumerator(
                        "SELECT result FROM " + Company.Kind.GetInstance<Company.Kind>().FullInstanceClassName + " result WHERE result.OrgRegNo=variable(String, orgregno)"))
                    {
                        sqlEnumerator.SetVariable("orgregno", orgregno);

                        while (sqlEnumerator.MoveNext())
                        {
                            company = sqlEnumerator.Current as Company;
                        } 
                    }

                    if (company == null)
                    {
                        company = New() as Company;
                        company.OrgRegNo = orgregno;
                ...
                {
                    throw new NullReferenceException("Cannot create a new company without a OrgRegNo");
                }
        ...
        {
            using (SqlEnumerator sqlenum = Sql.GetEnumerator("SELECT result FROM " + Subsidiary.Kind.GetInstance<Subsidiary.Kind>().FullInstanceClassName + " result WHERE result.ToWhat=variable(" + Company.Kind.GetInstance<Company.Kind>().FullInstanceClassName + ", company)"))
            {
